...
function afficherProjets(listeProjets) {
    ...
    document.querySelectorAll('.btn-supprimer').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        // Détecte le clique sur le bouton supprimer
    
        const index = projets.findIndex(p => p.id === id);
        projets.splice(index, 1);
        //supprime le projet quand le clic est détecté
    
        afficherProjets(projets);
        sauvegarder();
        //Sauvegarde la page quand le projet est supprimé et la met à jour
...
form.addEventListener('submit', (event) => {
  ...
  sauvegarder(); //Sauvegarde notre projet sur le site
});
